---
title: "PAC 1_Informe _Julia Roosjen"
...
Los objetivos de esta pac es planificar y executar una versión simplificada de un proceso de analisi de datos metabolomicos. 

Por lo que respecta al ensayo, el objetivo de este seria determinar las consequencias de un trasplante de microbiota fecal (FMT) a nivel metabolomico. Dado que el FMT se utiliza para restaurar la microbiota intestinal. Esta tecnica consiste en la administración de una solucion de materia fecal de un donante al tracto intestinal para modificar la composicion de la microbiota intestinal del receptor del trasplante. Este trasplante normamlente tiene efectos directos en el metabolismo intestinal. 

<https://pmc.ncbi.nlm.nih.gov/articles/PMC4749851/>

## Metodos

Amb què heu treballat. Variarà segons el cas, però habitualment
contindrà: origen (font) i naturalesa (tipus) de les dades, metodologia
emprada, eines estadístiques i bioinformàtiques utilitzades, procediment
general d'anàlisi, etc.
...
Adicionalmente a la estructura de los datos tambén se realiza un resumen estadistico de los datso con `summary()`
...
Para realizar un analisis mas completo  de los datos, se han normalizado dichos datos utilizando como referencia la información del siguiente enlace: <https://bioconductor.org/packages/release/bioc/vignettes/POMA/inst/doc/POMA-workflow.html> (Se encuentra dentro del temario)
...
Una vez generado processed se generar el grafico de **análisis de componentes principales (PCA)** sobre los datos contenidos en el objeto `processed.` Este gráfico representa la variabilidad de los datos de metabolitos en dos dimensiones principales. 
...
Para ver algunos de los valores estadisticos del conjunto crearemos dos dataframes para las muestras tomadas antes y depues del trasplante.
...
Estos valores estadisticos podrian caompararse para generar conclusiones sobre cual es el metabolito cuyos niveles sufren un mayor cambio l realizar el trasplante. También podriamos realizar un test ANOVA de los datos (transformandolos) para averigual que metabolitosse ven mas o menos influenciados por el transplante. 


## Referencias

quí haureu d'incloure un enllaç al repositori de GitHub que
conté el codi que heu utilitzat per abordar l'anàlisi
